 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-re/index
 * @returns eslint-config-re/react
 * @returns eslint-config-re/typescript/index
 * @returns eslint-config-re/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'eslint-config-re/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
